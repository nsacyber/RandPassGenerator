...
import gov.nsa.ia.gen.HexKeyGen;
 ...
 * @author nziring
 *
 *         Updated 20180925 by amsagos Updated 20211204 by
 *         nlzirin
 */
...
public class RandPassGenerator {
	...
	/**
	 * Default strength of keys to generate, in bits. (Note that this
	 * is not related to the strength of the underlying DRBG, which
	 * is fixed and defined in RandManager.java.)
	 */
	 ...
	 * 
	 * @param logfile Path to a writeable log file, null to log to
	 *                stderr
	 * @param pw      A usable printwriter for output, may not be
	 *                null
	...
	public RandPassGenerator(String logfile, PrintWriter pw, boolean verbose) {
		...
		if (logfile != null) {
			...
			try {
				...
				logger.warning(
						"RandPassGenerator - could not open log output file " + logfile + " - all logging to console.");
			}
		}
		...
		if (DEFAULT_KEYLOGFILE != null) {
			...
			try {
				...
				KeyLogger.warning("RandPassGenerator - could not open key log output file " + DEFAULT_KEYLOGFILE
		...
		try {
			...
			// 1.2 UPDATE: SP800-90A 8.6.7 states nonce should be a
			// random value with at
			// least (security_strength/2) bits of entropy.
			// The value could be acquired from the same source and at
			// the same time as the
			// entropy input (primarySource). Instead of saving entropy
			// to a
			// file during shutdown for later use as the nonce entropy
			// source or using the
			// system time when an entropy file doens't exist, use same
			// entropy source
			// as primary entropy input for the DRBG seed. Also removes
			// any concerns about
			// saving entropy used for the DRBG seed on the system.
			...
			// call the static known answer test on our DRBG class, to
			// make sure it works
			...
			startmessage(
					"Initializing randomness; this will take some time if system entropy isn't full.  Be patient, or go do something else on this computer to help the system gather more entropy.  Thank you.");
	...
	/**
	 * Utility function for making passwords or hex keys easier to
	 * read by adding in separators every so many characters. This
	 * method accepts a source string, a separator string, and a
	 * number of characters per group, and returns a string with
	 * groups of characters from the source separated by the
	 * separator. The last group might be short, if there are too few
	 * characters in the source.
	 *
	 * @param src     source string
	 * @param grpsize number of chars per group, must be positive,
	 *                usual value is 4
	 * @param sep     separator string, usually a single character
	 *                like " " or "-"
	 * @return a string of the chars from src, with interspersed
	 *         separators
	 */
	...
	/**
	 * Set up use of chunk formatting for generating keys and
	 * passwords. (Chunk formatting doen not apply to passphrases).
	 * To disable chunk formatting, set first parameter to 0 or
	 * negative.
	 *
	 * @param cs   chunk size to use, positive (supply 0 to disable
	 *             chunking)
	...
	public void setChunkFormatting(int cs, String csep) {
		if (cs <= 0) {
			...
			chunkSep = csep;
			if (chunkSep == null)
				chunkSep = DEFAULT_CHUNK_SEPARATOR;
		}
	}

	/**
	 * Generate some passwords of a specified strength using a
	 * specified character set, or the default character set. All
	 * passwords will be output to stdout. To specify a character
	 * set, the supplied string is examined, a lowercase letter in
	 * the string causes the password to have lowercase letters,
	 * uppercase letter adds uppercase, digit adds digits, anything
	 * else adds basic punctutation.
	 * 
	 * If the charsetfile argument is supplied, then it is read and
	 * used to create the custom charset. Note that charsets and
	 * charsetfile should not both be supplied, but if they are then
	 * charsetfile takes precedence.
	 *
	 * @param count       how many passwords to generate, positive
	 * @param strength    password strength in bits, usually 128,
	 *                    160, or 256
	 * @param charsets    which charsets to use in the password, if
	 *                    null then use default
	 * @param charsetfile a custom charset file to read, if null then
	 *                    use default
	...
	public int generatePasswords(int count, int strength, String charsets, String charsetfile) {
		...
		if (strength > (drbg.getStrength() * 2)) {
			logger.warning("RandPassGen - requested password strength " + strength
		...
		if (charsetfile != null) {
			...
			try {
				...
				logger.severe("RandPassGen - unable to read custom character set file '" + charsetfile
						+ "', cannot proceed.");
	...
	/**
	 * Generate some usernames from specified prefix set, or the
	 * default character set with a 4-digits suffix. All usernames
	 * will be output to stdout.
	 * 
	 * @param count        how many usernames to generate, positive
	 * @param prefixlist   URL to the prefixlist to use, or null for
	 *                     default
	 * @param maxPrefixLen maximum length of a word to use in the
	 *                     username, <3 means use default
	 * @param ruc          random upcase the first ruc letters, 0 or
	 *                     positive
	 * @return number of usernames generated, or -1 on error.
	 */
	public int generateUsernames(int count, URL prefixlist, int maxPrefixLen, int ruc) {
		CharacterSet cs;
		WordSet ws;
		AbstractDRBG drbg;

		if (count < 1) {
			logger.warning("RandPassGen - count for usernames < 1, error");
		...
		ws = new WordSet(prefixlist, logger);
		int maxlen = maxPrefixLen;
		...
		logger.fine("RandPassGen - initialized username word set, size=" + ws.size());

		cs = new CharacterSet(logger);
		cs.addSet(CharacterSet.DIGITS);

		logger.fine("RandPassGen - initialized username suffix digits set, size=" + cs.size());
		if (verbose) {
			message("RandPassGen - initialized username suffix digits set, size=" + cs.size());
		}
		...
		for (i = 0; i < count; i++) {
			...
			if (username == null) {
				logger.warning("RandPassGen - word set username generator failed!  Error.");
				return -1;
			} else {
				...
				for (String s : username) {
					...
					if (wrd == null) {
						logger.warning("RandPassGen - fatal error in trying to randomly upcase a username");
						return -1;
					}
					String digits = cs.getRandomStringByEntropy(10, drbg);
					// add word and digits to the username
					sj.add(wrd + digits);
				}
				users.add(sj.toString());
			}
		}

		logger.info("RandPassGen - generated " + users.size() + " usernames");

		for (String p : users) {
			outputWriter.println(p);
		}
		outputWriter.flush();
		logger.info("RandPassGen - output usernames to designated output channel");
	 ...
	 * default word set. All passphrases will be output to stdout.
	 *
	 * @param count      how many passphrases to generate, positive
	 * @param strength   passphrase strength in bits, usually 128,
	 *                   160, or 256
	 * @param wordlist   URL to the wordlist to use, or null for
	 *                   default
	 * @param maxWordLen maximum length of a word to use in the
	 *                   passphrase, <3 means use default
	 * @param ruc        random upcase the first ruc letters, 0 or
	 *                   positive
	...
	public int generatePassphrases(int count, int strength, URL wordlist, int maxWordLen, int ruc) {
		...
		if (strength > (drbg.getStrength() * 2)) {
			logger.warning("RandPassGen - requested passphrase strength " + strength
					+ " is greater than hash size of underlying DRBG.  Error.");
			message("Error - requested passphrase strength of " + strength
					+ " is greater than the hash size of the underlying DRBG");
			return -1;
		}
	...
	/**
	 * Generate some raw hexadecimal keys of specified strength. All
	 * keys will be output to stdout.
	 *
	...
	public int generateKeys(int count, int strength, boolean enc) {
		...
		if (strength > (drbg.getStrength() * 2)) {
			logger.warning("RandPassGen - requested key strength " + strength
					+ " is greater than hash size of underlying DRBG.  Error.");
			message("Error - requested key strength of " + strength
					+ " is greater than the hash size of the underlying DRBG");
			return -1;
		}
		...
		for (i = 0; i < count; i++) {
			...
			if (rawkey == null) {
				...
				// hash the generated keys to use for KEY_ID and add to the
				// key generation
				// transaction log
		...
		for (String p : keys) {
			...
			// if user asked for password encryption, create an
			// encrypted file for
	 ...
	 * needs a rewrite. Original version read a password with no
	 * display, once, so user couldn't see it!
	 * 
	 * Key File Encryption Prompt. Encrypts generated key to a file
	 * using a password types by the user on system console.
	 *
	...
	/***
	 * Decrypt an encrypted key file, using a password read from the
	 * console.
	 *
	 * TODO: the encrypt/decrypt functionality is seriously crufty
	 * and needs a rewrite.
	 * 
	 * Key File Decryption Prompt. Decrypts user provided file using
	 ...
	 * 
	 * Generates KeyID of key using first 64-bits of Hash and current
	 * date (YYYYMMDD_hhmmssH1H2H3H4H5H6H7H8H9H10H11H12H13H14H15H16).
	 * 
	...
	private static OptionManager makeOptions() {
		...
		// ret.addOption("randfile", "Path to file to use for saved
		// entropy", true,
		// "-randfile", DEFAULT_RAND_FILE);
		...
		ret.addOption("customfile", "File containing an explicit custom charset to use in passwords", true, "-pwcustom",
				null);
		ret.addOption("passphrases", "Number of random passphrases to generate", true, "-pp", null);
		ret.addOption("usernames", "Number of random usernames to generate", true, "-un", null);
		...
		ret.addOption("usernameURL", "URL of a list of prefixs to use in usernames", true, "-unurl", null);
		ret.addOption("outfile", "Filename or path to which output should be written; otherwise output goes to stdout",
				true, "-out", null);
	...
	/**
	 * Main method for the RandPassGenerator application - this
	 * method processes the command-line args, then initializes the
	 * RandPassGenerator and the RandManager, then generates the
	 * requested values.
	 * 
	 * This method uses
	 */
	public static void main(String[] args) {
		...
		int numUsernames = opt.getValueAsInt("usernames");
		...
		String unurl = opt.getValue("usernameURL");
		...
		if (rpg != null) {
			...
			if (numUsernames > 0) {
				...
				cnt = rpg.generateUsernames(numUsernames, unURL, maxWordLen, randUpcase);
				if (cnt <= 0) {
					rpg.message("Failed to generate usernames");
					rpg.getLogger().warning("Tried to generate " + numUsernames + " usernames, but failed.");
				} else {
					rpg.message("Generated " + cnt + " usernames");
				}
			}
